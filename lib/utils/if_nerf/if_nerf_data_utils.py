...
def sample_ray_h36m(img, msk, K, R, T, bounds, nrays, split, bounds_big=None):
    H, W = img.shape[:2]
    ray_o, ray_d = get_rays(H, W, K, R, T)
    ...
    _, _, mask_at_box_eval = get_near_far(bounds, ray_o.reshape(-1, 3).astype(np.float32), ray_d.reshape(-1, 3).astype(np.float32))
    ...
    return rgb, ray_o, ray_d, near, far, coord, mask_at_box, mask_at_box_eval
